...
from image_generation import create_animated_gif, create_animated_gif_hotshot_xl
...
def create_video(audio_path, full_video=True, video_length=0, video_fps=16, video_style=VIDEO_STYLE, clip_method="Loop", negative_prompt=NEGATIVE_PROMPT, generate_lyric_prompts=True, useLocalGeneration=True, video_prompt_style=VIDEO_PROMPT_STYLE):
    # load audio
    ...
    if generate_lyric_prompts == True:
        ...
        for lyric in audio_transcription:
            ...
            lyric["prompt"] = text_generation(lyric["text"], useLocalGeneration, video_style)
            print("\nlyric: ", lyric["text"], "\nstart: ", lyric["start"], "\nend: ", lyric["end"], "\nprompt: ", lyric["prompt"])
    ...
    for index, segment in enumerate(audio_transcription):
        ...
        if generate_lyric_prompts == True:
            ...
            create_animated_gif_hotshot_xl(segment["prompt"], 832, 512, video_clip_save_path, video_fps, negative_prompt, video_prompt_style)
        else:
            print("generating gif with lyric.")
            create_animated_gif_hotshot_xl(segment["text"]+", "+video_style, 832, 512, video_clip_save_path, video_fps, negative_prompt, video_prompt_style)

        final_clip_save_path = "./video_clips/"+str(index)+"final.mp4"

        if clip_method == "Loop":
            loop_clip((segment["end"]-segment["start"]), video_clip_save_path, final_clip_save_path, video_fps/2)
        if clip_method == "Stretch":
            stretch_clip((segment["end"]-segment["start"]), video_clip_save_path, final_clip_save_path, video_fps/2)
    ...
    file_count = len([f for f in os.listdir("./finished_videos") if os.path.isfile(os.path.join("./finished_videos", f))])

    final_video.write_videofile("./finished_videos/"+str(file_count)+".mp4", audio_codec="aac", fps=video_fps)
